...
%Compute the "error_factor" by multiplying the movie rating error by the R matrix.
...
diff_sq = error_factor.^2;

J = 0.5 * sum(sum (diff_sq));
...
%Colabrative Filtering Gradient (w/o regulariztion)
X_grad = error_factor * Theta ;
Theta_grad = error_factor' * X ;

%Colabrative Filtering Gradient (w/ regulariztion)
reg_x_grad = lambda * X;
